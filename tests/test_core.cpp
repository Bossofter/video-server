...
int test_core() {
  ...
  latest = server.get_latest_frame_for_stream(cfg.stream_id);
  assert(latest.has_value());
  assert(latest->frame_id == 2);
  assert(latest->timestamp_ns == 200);
  assert(latest->width == 2);
  assert(latest->height == 2);
  // Rainbow mapping for grayscale should produce colored output for mid values.
  assert(!(latest->bytes[3] == latest->bytes[4] && latest->bytes[4] == latest->bytes[5]));
  ...
  assert(!server.get_latest_frame_for_stream("missing").has_value());
