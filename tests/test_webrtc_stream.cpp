...
namespace
{
    ...
    TEST(WebRtcStreamSessionTest, EncodedUnitDeliveryPathConsumesLatestH264Unit)
    {
        ...
        video_server::WebRtcStreamSession session(
            "stream-a", [](const std::string &)
            { return std::shared_ptr<const video_server::LatestFrame>{}; },
            [latest](const std::string &)
            { return latest; });
    ...
    {
        video_server::WebRtcStreamSession session(
            "stream-stop", [](const std::string &)
            { return std::shared_ptr<const video_server::LatestFrame>{}; },
            [](const std::string &)
            { return std::shared_ptr<const video_server::LatestEncodedUnit>{}; });
